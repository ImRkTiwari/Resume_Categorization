...
if uploaded_file is not None:
    try:
        ...
        if uploaded_file.type == "text/plain":
            ...
            # If PDF, extract text (use PyPDF2 or pdfplumber here for better extraction)
            import PyPDF2
        ...
        if not resume_text.strip():
            st.error("No text could be extracted from the uploaded resume. Please upload a valid file.")

        # Clean the text
        cleaned_resume = cleanResume(resume_text)

        # Show extracted and cleaned text (optional, for debugging)
